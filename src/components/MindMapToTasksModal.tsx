...
interface GraphNode { id: string; text: string }
...
export function MindMapToTasksModal({
    ...
    (async () => {
      try {
        const [res, projs] = await Promise.all([
          api<{ tasks: { title: string }[]; source: 'ai' | 'rule' }>(
            '/ai/mindmap-to-tasks', { method: 'POST', body: { nodes, edges } }),
  ...
  function toggle(i: number) { setTasks((ts) => ts.map((t, j) => (j === i ? { ...t, sel: !t.sel } : t))); }
  ...
  return (
    <div className="fixed inset-0 z-[60] overflow-y-auto overlay-in" style={{ background: 'rgba(0,0,0,0.45)' }} onClick={onClose}>
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="w-full max-w-md modal-in rounded-2xl border border-slate-200/80 dark:border-white/10 shadow-2xl overflow-hidden"
          style={{ background: 'var(--bg-page)' }} onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex items-start justify-between gap-3 px-5 pt-5 pb-3">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <div className="text-base font-bold text-slate-800 dark:text-white/90 leading-tight">Turn notes into tasks</div>
                <div className="text-[11px] text-slate-400 dark:text-white/35">
                  {loading ? 'Reading your mind map…'
                    : source === 'ai' ? 'Refined from your notes' : 'Pulled straight from your notes'}
                </div>
              </div>
            </div>
            <button onClick={onClose} className="p-1 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-white/70 transition-colors shrink-0">
              <X size={16} />
            </button>
          </div>
          ...
          {doneCount !== null ? (
            <div className="px-5 py-8 text-center">
              ...
              <div className="text-sm font-bold text-slate-700 dark:text-white/85">Added {doneCount} task{doneCount === 1 ? '' : 's'}</div>
              <div className="text-xs text-slate-400 dark:text-white/35 mt-1">They’re on the project now — close to keep brainstorming.</div>
              <button onClick={onClose} className="btn-primary mt-5 text-sm">Done</button>
            </div>
          ) : (
            <>
              <div className="px-5 max-h-[46vh] overflow-y-auto">
                {loading ? (
                  <div className="space-y-2 py-2">
                    {[0, 1, 2].map((i) => <div key={i} className="h-9 skeleton rounded-lg" />)}
                  </div>
                ) : tasks.length === 0 ? (
                  ...
                  <div className="space-y-1.5 py-1">
                    {tasks.map((t, i) => (
                      <button key={i} type="button" onClick={() => toggle(i)}
                        ...
                        <span className={`mt-[1px] w-[18px] h-[18px] rounded-[5px] border flex items-center justify-center shrink-0 transition-colors ${
                        ...
                        <span className="text-sm text-slate-700 dark:text-white/80 leading-snug">{t.title}</span>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <div className="px-5 py-4 mt-1 border-t border-slate-100 dark:border-white/[0.06] space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <label className="text-[11px] font-bold uppercase tracking-wide text-slate-400 dark:text-white/35 shrink-0">Add to</label>
                  <select
                    value={projectId}
                ...
                {isLead && (
                  <label className="flex items-center gap-2 text-xs text-slate-500 dark:text-white/50 cursor-pointer select-none">
                    <input type="checkbox" checked={privateToMe} onChange={(e) => setPrivateToMe(e.target.checked)}
                      className="rounded border-slate-300 dark:border-white/20" />
                    Keep these private to me (don’t show on the team board)
                  </label>
                )}
                <button onClick={createAll} disabled={creating || loading || selectedCount === 0 || !projectId}
