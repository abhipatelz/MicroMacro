...
/**
 * Browser-only first-run admin setup. Visible at /bootstrap.
 *
 * Operationally:
 *   1. Set ADMIN_BOOTSTRAP_TOKEN=<some long random string> in your hosting
 *      env (Vercel → Settings → Environment Variables). Redeploy.
 *   2. Open /bootstrap, paste the token, pick an email + password.
 *      Optionally tick the cleanup checkboxes to wipe pre-launch seed data
 *      in the same call.
 *   3. After success, delete the env var and redeploy. The endpoint then
 *      404s and the page is inert until you reactivate it.
 *
 * This page never reads or echoes the token — paste-only, never logged.
 */
          ...
          <div className="space-y-4">
            ...
            <div className="border-t pt-4 space-y-2 text-sm">
              <p className="text-slate-700">
                ...
                {' '}from your hosting environment now, then redeploy. This page will then
                stop working until you set the token again.
              </p>
              <Link href="/login"
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="border-t pt-4 space-y-2">
              <Check value={cleanupUsers} onChange={setCleanupUsers}
                     ...
                     hint="Keeps only the new admin + anyone who joined via a consumed invite. Founder/seed accounts are removed." />
            ...
            <p className="text-xs text-slate-500 text-center">
              If you see "Bootstrap is disabled", set{' '}
              <code className="px-1 py-0.5 bg-slate-100 rounded">ADMIN_BOOTSTRAP_TOKEN</code>{' '}
              in your hosting env and redeploy.
            </p>
          </form>
        )}
      </div>
