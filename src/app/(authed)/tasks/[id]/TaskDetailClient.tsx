...
export default function TaskDetailClient(props: TaskDetailClientProps) {
  ...
  async function addSubtask() {
    if (!canAddSubtask || !newSub.trim()) return;
  ...
  const canToggleSubtask = isLead || isAssignee;
  const canAddSubtask = isLead;
          ...
          <div className="space-y-1">
            {subtasks.map((s: any) => (
              <div key={s.id} className="flex items-center gap-2.5 text-sm py-1 group">
                <button
                  onClick={() => canToggleSubtask && toggleSub(s)}
                  ...
                  } ${canToggleSubtask ? '' : 'opacity-60 cursor-default'}`}
                >
