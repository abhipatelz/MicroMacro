...
function SignupForm({ token, initialState }: { token: string; initialState: InviteState }) {
  const router = useRouter();

  const [phase,    setPhase]    = useState<'invalid' | 'ready' | 'submitting' | 'done'>(
    initialState.valid ? 'ready' : 'invalid'
  );
  const [reason,   setReason]   = useState(initialState.reason || '');
  const [email,    setEmail]    = useState(initialState.email || '');
  const [invitedBy]              = useState(initialState.invitedByName || '');
...
export default function SignupClient({ token, initialState }: { token: string; initialState: InviteState }) {
  return (
    <div className="min-h-screen flex items-center justify-center p-6"
      ...
      <div className="w-full max-w-[420px]">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 px-6 py-7"
          ...
          <SignupForm token={token} initialState={initialState} />
        </div>
