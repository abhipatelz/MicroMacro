...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // The year build scans every completed task + subtask of the user — the
    // heaviest read behind the Activity graph. Read-through cached for 2 min
    // (keyed by viewer privacy lens, since self-views include personal
    // projects and team views scrub them). No-op without Redis env vars.
    const payload = await cached(
      `activity:${params.id}:${year}:${viewingSelf ? 'self' : 'team'}`,
      120,
      async () => {
