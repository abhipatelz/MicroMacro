 ...
 * project-detail lists both use this lightweight position field; visibility is
 * not enough to mutate order: only a lead/admin or the project owner may
 * reorder. We write position = array index so the order is exact and stable.
 */
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!isLead(user!.role) && !ownsProject) {
      return NextResponse.json({ error: 'Only a lead or project owner can reorder tasks.' }, { status: 403 });
    }
